...
/*
- Build a function that passes a string of email data

The Email Message String :
- the string contains multiple email messages
  - each message is separated by the delimiter string ##||##
- Each message has five parts
...
In: email data (in string form)
Out: log to the console
      - Count of Email
      - date range

PROBLEM:
- we have to break down data into chunks before we can work on it.
- First: break data into email messages
*/

function mailCount(emailData) {
  let emailMessages = emailData.split('##||##').map(email => email.trim());
  let dates = emailMessages.map(email => {
    return email.split('#/#')[2].trim().split(' ')[1];
  }).map(date => new Date(date)).sort();

  dates = dates.sort((a, b) => a.valueOf() - b.valueOf());

  console.log('Count of Email: ' + emailMessages.length);
  console.log(dates[0].toDateString() + ' - ' + dates[dates.length - 1].toDateString());
}
